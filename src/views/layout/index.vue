<template>
  <div>
    <document>
      ...
      <ul>
        <li>大色块强调 — 建议用于底色为深色系时，当前页面父级的导航项。</li>
        <li>高亮火柴棍 — 当导航栏底色为浅色系时使用，可用于当前页面对应导航项，建议尽量在导航路径的最终项使用。</li>
        <li>字体高亮变色 — 从可视化层面，字体高亮的视觉强化力度低于大色块，通常在当前项的上一级使用。</li>
        <li>字体放大 — <code>12px</code>、<code>14px</code> 是导航的标准字号，<code>14px</code> 字体用在一、二级导航中。字号可以考虑导航项的等级做相应选择。</li>
      </ul>
      <h3>组件概述</h3>
      <ul>
    ...
    <example-basic-usage />
    <example-vertical />
    <example-header-sider-first />
    <example-header-sider-second />
    <example-sider />
    ...
    <document>
      ...
      <table class="example-api-props">
        ...
        <tbody>
          ...
          <tr>
            <td>breakpoint</td>
            <td>触发响应式布局的断点，可选值为 <code>xs</code>、<code>sm</code>、<code>md</code>、<code>lg</code>、<code>xl</code>、<code>xxl</code> 或者不设</td>
            <td>String</td>
            <td>--</td>
          </tr>
          <tr>
...
<script>
  ...
  import ExampleHeaderSiderFirst from "./examples/header-sider-first";
  import ExampleHeaderSiderSecond from "./examples/header-sider-second";
  import ExampleSider from "./examples/sider";
  import ExampleCustomTrigger from "./examples/custom-trigger";
  ...
  export default {
    ...
    components: {
      ...
      ExampleBasicUsage,
      ExampleVertical,
      ExampleHeaderSiderFirst,
      ExampleHeaderSiderSecond,
      ExampleSider,
      ExampleCustomTrigger,
      ExampleResponsive
    }
  };
</script>