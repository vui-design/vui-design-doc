<template>
	<vui-doc-example v-bind:code="code" id="example-modal-asynchronously-close">
		<template slot="demo">
		...
		<template slot="description">
			...
			<p>当该属性为 <code>true</code> 时，点击按钮后，按钮会进入 <code>loading</code> 状态，同时事件回调函数会接收一个 <code>done</code> 函数作为参数，需要在异步任务完成后调用这个 <code>done</code> 函数，告知对话框异步任务执行完成，此时按钮会移除 <code>loading</code> 状态。</p>
			<p>1、<code>done()</code> 或 <code>done(true)</code> 表示 <code>loading</code> 状态结束后自动关闭对话框；</p>
			<p>2、<code>done(false)</code> 表示 <code>loading</code> 状态结束后禁止关闭对话框。</p>
		</template>
	</vui-doc-example>
</template>
