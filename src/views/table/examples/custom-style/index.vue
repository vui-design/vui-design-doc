<template>
  <example v-bind:code="code" id="example-table-custom-style">
    <template slot="demo">
    ...
    <template slot="title">特定样式</template>
    <template slot="description">
      <p>行：通过 <code>rowClassName</code> 属性可以给某一行指定一个样式类名；</p>
      <p>列：通过给 <code>columns[n]</code> 列选项添加 <code>className</code> 属性可以给某一列指定一个样式名称；</p>
      <p>单元格：通过给 <code>data[n]</code> 数据项添加 <code>cellClassName</code> 属性可以给任意一个单元格指定样式。</p>
    </template>
  </example>
</template>
